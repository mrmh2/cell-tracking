...
void CalculateLcoefficients()
{
  ...
  //We divide by the overall area to make it area-invariant
    ...
    EXTRAS[c].L[k]/=sqrt(EXTRAS[c].cellarea);
...
int main(int argc,char *argv[])
{
 ...
 // int k=1,lobe=1;
  ...
  //
    CELLS(reconstructedcells,
        if (EXTRAS[c].timepoint==0) count++;
    )
  
  fp = fopen("lcoeffs.txt", "w");
 // CELLS(reconstructedcells,
  for(c=1;c<700;c++) {
        if (EXTRAS[c].timepoint==0) {
            fprintf(fp, "%d:", EXTRAS[c].oldsigma);
            //printf("%d: Extras[%d].timepoint=%d\n", EXTRAS[c].oldsigma, c, EXTRAS[c].timepoint);
            for (k=1;k<inoofharmonicsanalyse;k++) fprintf(fp, " %f ", EXTRAS[c].L[k]);
             fprintf(fp, "\n");
        }
  }
  //)
